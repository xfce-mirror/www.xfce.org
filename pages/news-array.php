...
$news = array (
  ...
  array ('title' => R_('Xfce 4.18 released'),
     ...
     'paragraphs' => array (
             ...
             '<img src="'. $static_media .'/about/screenshots/4.18-1.png" border="0" alt="Xfce 4.18 screenshot" style="max-width: 100%;"/>',
  ...
  array ('title' => R_('Xfce 4.16 released'),
     ...
     'paragraphs' => array (
             ...
             '<img src="'. $static_media .'/about/tour/4.16/upstream-icons.png" border="0" alt="upstream-icons" style="max-width: 100%;"/>',
             R_('And then finally we did some housekeeping (e.g. by dropping Gtk2 support code) and we also added features and improvements - all while sticking to a reasonable release cycle timeframe.'),
             ...
             array(
                     '<ul>',
                             ...
                             '<li>'.R_('We added a new plugin to the <a href="https://docs.xfce.org/xfce/xfce4-panel/start">panel</a> dubbed "statustray" which combines both StatusNotifier and legacy Systray items. An animated autohide transition now clearly shows the user "where the panel has gone" and a dark mode (enabled by default) makes the panel drop into the background a bit more with bright themes like the Gtk default Adwaita. Quite a few more smaller improvements were also introduced like accessing desktop actions from a Launcher´s right-click menu, the Window Buttons plugin now offering to `Launch a new instance` of an application and the Workspace Switcher now optionally showing automatic numbering for workspaces.').'</li>',
                             '<li>'.R_('Support for fractional scaling was added to the <a href="https://docs.xfce.org/xfce/xfce4-settings/display">display dialog</a>, along with highlighting the preferred mode of a display with an asterisk and adding aspect ratios next to resolutions. Falling back to a working mode after misconfiguring the display layout has also been made more robust.').'</li>',
                             '<li>'.R_('A new tab in the "About Xfce" dialog shows basic system information like CPU or GPU type. Furthermore the layout and visuals of the dialog were improved.').'</li>',
                             '<li>'.R_('The <a href="https://docs.xfce.org/xfce/xfce4-settings/start">settings manager</a> has improved search and filter capabilities. Furthermore all settings dialogs now use window decorations drawn by Gtk (client side decorations).').'</li>',
                             ...
                             '<li>'.R_('Our file manager <a href="https://docs.xfce.org/xfce/thunar/start">Thunar</a> received a boatload of fixes and quite a few notable features, including pause for copy/move operations, support for queued file transfer, remembering view settings per directory and support for transparency in Gtk themes.').'</li>',
                             '<li>'.R_('Our <a href="https://docs.xfce.org/xfce/thunar/tumbler">thumbnailing service tumbler</a> became more flexible by being able to exclude paths. Furthermore support for .epub (e-book format) was added.').'</li>',
                             '<li>'.R_('The <a href="https://docs.xfce.org/xfce/xfce4-session/start">session manager</a> offers improved support for GPG agent 2.1 and the setting dialog was visually improved.').'</li>',
		...
		'paragraphs' => array (
			...
			array(
				'<ul>',
					...
					'<li>'.R_('The <a href="https://docs.xfce.org/xfce/xfce4-appfinder/start">application finder</a> can now optionally be opened as a single window and can now be more easily navigated with the keyboard only.').'</li>',
			...
			array(
				'<ul>',
					...
					'<li>'.R_('Our image viewer <a href="https://docs.xfce.org/apps/ristretto/start">Ristretto</a> has seen various user interface improvements and support for setting the desktop wallpaper. It has recently also seen its first Gtk3-based development release.').'</li>',
	...
	array ( 'title' => null,
		...
		'paragraphs' => array (
			...
			R_('Among other things we updated the default keyboard shortcuts to include monitor keys. The status icon of the file manager\'s progress dialog is now properly hidden when the dialog is destroyed. Its wallpaper plugin is now capable of handling filenames with spaces. The environment variables of commands launched from the Xfce run dialog were not set properly, so we fixed that so that commands launched from the dialog always inherit the environment of the Xfce session.'),
