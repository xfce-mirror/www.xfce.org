#!/usr/bin/env python3
"""Keep templates and i18n/en.json in agreement, and point po/ui.pot at templates.

i18n/en.json is hand-maintained and is the source of truth for UI strings, so a
...
error from Hugo. This turns that into a build failure naming the string.
    ...
    --check      templates and i18n/en.json agree (run on every build)
    --annotate   rewrite po/ui.pot references to point at the templates,
                 replacing hugo-gettext's "i18n/en.json:0" (run after extract)
"""
...
def check(uses: dict, strings: dict) -> int:
    ...
    for key in missing:
        ...
        print(f'  ERROR: no i18n/en.json entry for {key!r} (used at {where})', file=sys.stderr)
    for key in unused:
        print(f'  WARNING: i18n/en.json has {key!r} but no template uses it', file=sys.stderr)

    if missing:
        print(f'\n{len(missing)} string(s) used by templates but absent from i18n/en.json.',
              file=sys.stderr)
        print('Add them there as {"<key>": {"other": "<English text>"}}, using '
              '{{ .s1 }}, {{ .s2 }} ... for anything the template passes in, '
              'then run ./build.sh --update-po.', file=sys.stderr)
        return 1

    print(f'  {len(uses)} UI strings, templates and i18n/en.json agree'
...
def annotate(uses: dict, strings: dict, pot_path: Path) -> None:
    ...
    # through en.json to recover the key each msgid came from.
...
def main() -> None:
    ...
    parser.add_argument('--check', action='store_true', help='verify templates match i18n/en.json')
    ...
    strings = json.loads((repo_root / 'i18n' / 'en.json').read_text(encoding='utf-8'))
