<h1>漫游Xfce 4.4.0</h1>
<p align="justify">
经过许多天的等待，Xfce4.4.0版本终于在万众瞩目中发布了。我会高亮标出刚加入最新稳定版的一些特性。
</p>
...
<p align="justify">
在4.0及4.2版本中被要求的最多的特性是提供在桌面上放置图标的支持。现在，在Xfce 4.4.0中，该特性终于被加入到桌面管理器<b>Xfdesktop</b>中。
</p>
...
桌面管理器还拥有一个CDE中的流行特性，在桌面上显示最小化程序图标。
当然，如果你喜欢一个干净的桌面的话，你可以一次性禁用所有桌面图标。
</p>
...
<b>Thunar</b>是从零开写的，旨在为Xfce提供一个易用且相当小巧的文件管理器。它的用户界面被设计成与GTK+ 2.4中的文件选择器非常相似。其它文件管理器例如<b>Nautilus</b>和<b>pcmanfm</b>也采用了相似的设计。
</p>
...
<p align="justify">
Xfce 4.4.0提供对于可移动设备及媒体的轻松访问。您只需将可移动介质插入驱动器，桌面以及<b>Thunar</b>的侧栏就会出现相应的图标。
</p>

<p align="center"><img src="/images/about/tour/xfce44-removable-volumes.png" alt="Removable Volumes" /></p>

<p align="justify">
点击图标将自动挂载卷。右击图标卸载卷或弹出驱动器。注意，迄今，改特性需要<a href="http://freedesktop.org/wiki/Software_2fhal">HAL</a>的支持以及2.6.x以上版本的Linux或6.x以上版本的FreeBSD(FreeBSD 4.x和5.x无须HAL，拥有有限的可移动媒体的支持).
</p>
...
你可以将<b>MousePad</b>看做和Windows上的<b>NotePad</b>一样的东西。它启动非常迅速，通常小于一秒，即便是在老机器上。
</p>
...
<p align="justify">
这个版本拥有增强的混合管理的特性，支持ARGB窗口透明、窗口阴影以及边框透明等等。
</p>
...
Xfce 4.4中的<b>Xfce4-panel</b>被彻底地重写，提供了<i>开箱即用</i>的多面板支持，可用下图所示的<b>Panel Manager</b>进行轻松配置。
</p>
...
在以往Xfce的版本中一个主要的问题是，所有的插件都需要与面板运行于相同的进程，一个插件的崩溃会引起整个面板的崩溃。如今，对于外部插件的支持已被加入到面板中。
</p>
...
既然现在已经支持多面板了，分离的<b>Xftaskbar4</b>和<b>Xfce4-iconbox</b>已不在需要。取而代之的是作为面板插件的taskbar和iconbox。
</p>

<p align="justify">
<a href="http://goodies.xfce.org/">Xfce
Goodies Project</a>上的很多额外的面板插件都已经随新面板更新了，同时加入了很多新的。例如，崭新的<b>xfce4-xfapplet-plugin</b>允许用户在Xfce面板上加载GNOME面板小程序。
</p>
...
<p align="justify">
新的时间管理程序<b>Orage</b>取代了随Xfce 4.2.0一同引进的<b>Xfcalendar</b>。<b>Orage</b>提供了许多特性使您能够高效管理您的时间。
</p>

<p align="center"><img src="/images/about/tour/xfce44-orage.png" alt="Orage" /></p>

<p align="justify">
<b>Orage</b>非常小巧易用，它提供了所有你能在<b>Outlook</b>或<b>Evolution</b>等大型日历程序中找到的重要功能。过去<b>Xfcalendar</b>使用传统的<tt>dbh</tt>格式存储设置，而<b>Orage</b>基于<tt>ical</tt>，所以更兼容其他日历程序。
</p>
...
<p align="justify">
虽然<b>Terminal</b>4.2时期就可用了，但作为核心组件还不够成熟。这一版本中，它被正式加入核心组件。
</p>

<p align="center"><img src="/images/about/tour/xfce44-terminal.png" alt="Terminal" /></p>

<p align="justify">
除了你所期望的作为一个终端模拟器的基本特性以外，它还包括许多好的附加特性，比如多标签、可定制工具栏，以及通过<i>隐藏选项</i>进行全方位的配置。通过上图可以看到，这个版本通过<b>Xfwm4</b>的混合管理提供了真透明的支持。
</p>
...
<p align="justify">
<b>Xfprint</b>，Xfce的打印管理程序，在此版本中有许多微小的改进。首先，<tt>a2ps</tt>转换器不再被强制，但仍被推荐。加入对<tt>CUPS</tt> 1.2的支持，<b>Xfprint</b>现已能通过<tt>CUPS</tt>-后端显示打印状态。
</p>
...
<p align="justify">
你可以看到虽然打印对话框仍与Xfce 4.2时有几分相似，但是内部工作机制改善了，尤其是对<tt>CUPS</tt>的支持。除了这些，打印管理的函数被移到了一个库中，所以应用程序可以使用这个API来访问打印配置。
</p>
...
<p align="justify">
Xfce 4.4.0实行新的<a href="http://freedesktop.org/wiki/Standards_2fautostart_2dspec">自启动规范</a> - 事实上，Xfce是第一个实行这个规范的桌面，只不过其它的比Xfce发布得早罢了。;-)
</p>
...
当初只在<b>Terminal</b>可用的首选应用程序框架，已被引入Xfce，所以用户无须再编辑shell profiles以决定要用那个浏览器和那个终端模拟器。目的是为了使改变应用程序类别的操作尽量简单(GNOME用户可能已经注意到GNOME采用了这个方法，因为这太简单了)。
</p>

<p align="center"><img src="/images/about/tour/xfce44-preferences-keyboard.png" alt="Keyboard Shortcuts" /></p>

<p align="justify">
在Xfce 4.2中存在着键盘快捷键的问题... Xfce 4.2限制了快捷键数量。在Xfce 4.4中，这种限制已经成为了历史，应用程序快捷键已从窗口管理器快捷键中分离出来了。
</p>
...
<p align="justify">
请将对此文章的评论发到我的<a href="http://xfce-diary.blogspot.com/2007/01/visual-tour-of-xfce-440.html">blog</a>上，如果你有关于Xfce 4.4.0的问题或在安装时遇到了麻烦，请使用<a href="/community/lists">xfce</a>的邮件列表。
</p>
...
<ul>
<li><a href="http://www.xfce.org/">Xfce网站</a></li>
<li><a href="http://thunar.xfce.org/">Thunar网站</a></li>
</ul>
