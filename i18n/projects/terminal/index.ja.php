<div style="float:right; margin:0 0 0.5em 0.5em;"><img src="/images/projects/terminal.png" alt="terminal" width="48" height="48" /></div>

<h1>ターミナル</h1>
<p>ターミナルは目新しく、軽く、メモリをあまり食わないタブと多数のウィンドウをサポートしたXfceデスクトップ環境用の
ターミナルエミュレータです。キーボードショートカット、画面、色などをすべて好きなように設定できます。</p>

<h2>外部リンク</h2>
<p><a class="external" target="_blank" href="http://www.os-cillation.com/index.php?id=42&amp;L=5">ホームページ</a></p>
<h3>Subversion</h3>
<p><a class="external" target="_blank" href="http://svn.xfce.org/index.cgi/xfce/browse/terminal/trunk">WebSVN</a><br />
  <a class="external" target="_blank" href="http://svn.xfce.org/svn/xfce/terminal/">SVN</a></p>
<p><code class="code">svn co http://svn.xfce.org/svn/xfce/terminal/trunk terminal</code></p>

<h2>作者</h2>
<ul>
  <li>Benedikt Meurer [benny at xfce.org]</li>
</ul>

